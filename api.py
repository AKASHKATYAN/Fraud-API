...
def preprocess(df):
    ...
    dept_std = df.groupby("department_id")["amount"].transform("std").replace(0,1)
...
def explain(row):
    reasons = []
    if row["amount_zscore_dept"] > 3: reasons.append("Unusually high amount for department")
    if row["vendor_amount_ratio"] > 3: reasons.append("Vendor amount spike")
    if row["is_night"] == 1: reasons.append("Night-time transaction")
    if row["is_weekend"] == 1: reasons.append("Weekend transaction")
    if row["vendor_txn_count"] > 50: reasons.append("High frequency vendor")
...
async def predict_fraud(file: UploadFile = File(...)):
    try:
        ...
        try:
            ...
            return {"error": "Failed to read CSV. Make sure it is a valid CSV file."}
        ...
        if not hasattr(model, "decision_function"):
            return {"error": "Model does not support decision_function"}
        ...
        if denom == 0:
            ...
            df["risk_score"] = ((df["anomaly_score"].max() - df["anomaly_score"]) / denom) * 100
        ...
        return {
            ...
            "results": df[[
                ...
                "fraud_flag","anomaly_score","risk_score","risk_level","explanation"
